...
VMWAddOnsTray::MessageReceived(BMessage* message)
{
	switch (message->what) {
		...
		{
			BString aboutText;
			aboutText.SetToFormat(B_TRANSLATE_COMMENT("%s, version %s\n"
					"© 2009, Vincent Duvert\n"
					"Distributed under the terms of the MIT License.",
					"First %s is the application name, second one is the version string."),
				APP_NAME, APP_VERSION);
			BAlert* alert = new BAlert(B_TRANSLATE("About"), aboutText.String(),
				B_TRANSLATE("OK"), NULL, NULL, B_WIDTH_AS_USUAL, B_OFFSET_SPACING, B_INFO_ALERT);
	...
	if (fBackdoor.OpenRPCChannel() == B_OK) {
		...
		(new BAlert(TRAY_NAME,
			B_TRANSLATE("Unable to communicate with VMware. Your VMware version may be too old."
			 ...
			 B_TRANSLATE("Cancel"), NULL, NULL, B_WIDTH_AS_USUAL, B_STOP_ALERT))
			->Go();
	}
}
	...
	if (askUser) {
		result = (new BAlert(TRAY_NAME,
					  B_TRANSLATE("Are you sure you want to quit?\n"
						"This will stop clipboard sharing (but not mouse sharing if it is"
						" running)."),
					  B_TRANSLATE("Cancel"),
					  B_TRANSLATE("Quit"),
